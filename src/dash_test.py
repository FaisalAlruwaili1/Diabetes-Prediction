...
# 0. diabetes rate by state (choropleth map)
us_state_abbrev = {
...
fig_map = px.choropleth(
    ...
    title="Distribution of Reported Diabetes Cases in US States",
    template="plotly_dark",
)
fig_map.update_layout(**{k: v for k, v in TEMPLATE["layout"].to_plotly_json().items() if k not in ["template"]})
fig_map.update_layout(title_x=0.5, title_font=dict(color=TEXT, size=24))
...
app.layout = html.Div(style={
    ...
    html.Div(style={"padding": "40px 48px"}, children=[
        ...
        # row 0 — state choropleth map (full width, top)
        html.Div([
